...
function ChessPiece({ piece, squareWidth }) {
  const color = piece?.[0] === "w" ? "white" : "black";
  return <span className={`sandip-chess-piece ${color}-piece`} style={{ fontSize: `${Math.max(28, squareWidth * 0.82)}px` }} aria-hidden="true">{PIECE_GLYPHS[piece?.[1]] || "♟"}</span>;
}

const CUSTOM_PIECES = Object.fromEntries(
  ["wP", "wN", "wB", "wR", "wQ", "wK", "bP", "bN", "bB", "bR", "bQ", "bK"].map((piece) => [piece, ({ squareWidth }) => <ChessPiece piece={piece} squareWidth={squareWidth} />])
);
...
function Game() {
  ...
  useEffect(() => {
    ...
    const onReady = ({ fen, clocks: serverClocks }) => { setGame(new Chess(fen)); setClocks(serverClocks || { w: DEFAULT_TIME.initial, b: DEFAULT_TIME.initial, frozen: { w: false, b: false }, timeControl: DEFAULT_TIME }); setGameStarted(true); setGameOver(false); setSelectedSquare(null); setStatus("Game started — White to move"); };
    const onUpdate = (state) => { setGame(new Chess(state.fen)); setSelectedSquare(null); if (state.clocks) setClocks(state.clocks); setMoves((previous) => [...previous, { color: state.move.color, san: state.move.san }]); if (state.move.captured) setCaptured((previous) => ({ ...previous, [state.move.color === "w" ? "white" : "black"]: [...previous[state.move.color === "w" ? "white" : "black"], state.move.captured] })); if (state.checkmate) { setStatus(`${state.turn === "w" ? "Black" : "White"} wins by checkmate!`); setGameOver(true); } else if (state.draw) { setStatus("Draw"); setGameOver(true); } else if (state.check) setStatus(`${state.turn === "w" ? "White" : "Black"} is in check`); else setStatus(`${state.turn === "w" ? "White" : "Black"} to move`); };
    const onClockUpdate = (state) => setClocks(state);
    const onOver = ({ winner, reason }) => { if (reason === "timeout") setStatus(`${winner} wins on time!`); else if (reason === "draw") setStatus("Draw agreed"); else if (reason === "owner cheat") setStatus(`${winner} wins by Owner Mode 😈`); else setStatus(`${winner} wins by ${reason}`); setGameOver(true); };
    ...
    socket.on("connect", onConnect); socket.on("disconnect", onDisconnect); socket.on("room-ready", onReady); socket.on("game-update", onUpdate); socket.on("clock-update", onClockUpdate); socket.on("game-over", onOver); socket.on("player-disconnected", onDisconnectPlayer); socket.on("draw-offer", onDrawOffer); socket.on("draw-result", onDrawResult);
    return () => { socket.off("connect", onConnect); socket.off("disconnect", onDisconnect); socket.off("room-ready", onReady); socket.off("game-update", onUpdate); socket.off("clock-update", onClockUpdate); socket.off("game-over", onOver); socket.off("player-disconnected", onDisconnectPlayer); socket.off("draw-offer", onDrawOffer); socket.off("draw-result", onDrawResult); };
  }, [socket]);

  useEffect(() => { if (!gameStarted || gameOver || !roomCode) return; const sync = () => socket.emit("clock-sync", { roomCode }); const timer = setInterval(sync, 500); sync(); return () => clearInterval(timer); }, [gameStarted, gameOver, roomCode, socket]);

  function getCurrentTimeControl() { if (!customMode) return selectedTime; const minutes = Math.min(Math.max(Number(customMinutes) || 1, 1), 60); const increment = Math.min(Math.max(Number(customIncrement) || 0, 0), 120); return { initial: minutes * 60, increment, label: `${minutes}+${increment}` }; }
  function createRoom() { if (!connected) return setStatus("Not connected to server"); const timeControl = getCurrentTimeControl(); socket.emit("create-room", { userId: OWNER_DISCORD_ID, timeControl }, (response) => { if (!response.success) return setStatus(response.error); setRoomCode(response.roomCode); setMyColor("w"); setIsOwner(Boolean(response.owner)); setOwnerMode(false); setGame(new Chess(response.fen)); setGameStarted(false); setGameOver(false); setMoves([]); setCaptured({ white: [], black: [] }); setClocks(response.clocks || { w: timeControl.initial, b: timeControl.initial, frozen: { w: false, b: false }, timeControl }); setStatus(`Room created — waiting for opponent • ${response.timeControl?.label || timeControl.label}`); }); }
  function joinRoom() { const code = inputCode.trim().toUpperCase(); if (!code) return setStatus("Enter a room code"); if (!connected) return setStatus("Not connected to server"); socket.emit("join-room", code, (response) => { if (!response.success) return setStatus(response.error); setRoomCode(response.roomCode); setMyColor("b"); setIsOwner(Boolean(response.owner)); setOwnerMode(false); setGame(new Chess(response.fen)); setGameStarted(true); setGameOver(false); setMoves([]); setCaptured({ white: [], black: [] }); setClocks(response.clocks || { w: DEFAULT_TIME.initial, b: DEFAULT_TIME.initial, frozen: { w: false, b: false }, timeControl: DEFAULT_TIME }); setStatus(`Joined room — ${response.timeControl?.label || "game"} starting`); }); }
  function ownerCheat(action) { if (!isOwner) return; socket.emit("owner-cheat", { roomCode, action }, (response) => { if (!response?.success) setStatus(response?.error || "Owner action failed"); }); }
  function sendMove(from, to, promotionPiece = "q") { if (!roomCode || !gameStarted || gameOver || !myColor || game.turn() !== myColor) return false; const test = new Chess(game.fen()); try { const move = test.move({ from, to, promotion: promotionPiece }); if (!move) return false; socket.emit("make-move", { roomCode, move: { from, to, promotion: promotionPiece } }, (response) => { if (!response.success) setStatus(response.error); }); return true; } catch { return false; } }
  function chooseMove(from, to) { if (!gameStarted || gameOver || game.turn() !== myColor) return; if (game.get(from)?.color !== myColor) return; const target = game.moves({ square: from, verbose: true }).find((move) => move.to === to); if (!target) return setSelectedSquare(from); if (target.promotion) return setPromotion({ from, to }); sendMove(from, to); }
  function handleSquareClick(square) { if (!gameStarted || gameOver || game.turn() !== myColor) return; if (!selectedSquare) { if (game.get(square)?.color === myColor) setSelectedSquare(square); return; } if (selectedSquare === square) return setSelectedSquare(null); if (game.get(square)?.color === myColor) return setSelectedSquare(square); chooseMove(selectedSquare, square); }
  function handlePieceDrop({ sourceSquare, targetSquare }) { if (!targetSquare) return false; setSelectedSquare(null); try { const move = game.moves({ square: sourceSquare, verbose: true }).find((m) => m.to === targetSquare); if (move?.promotion) { setPromotion({ from: sourceSquare, to: targetSquare }); return false; } return sendMove(sourceSquare, targetSquare); } catch { return false; } }
  function answerDraw(accepted) { socket.emit("answer-draw", { roomCode, accepted }); setDrawOffered(false); }
  function resetGame() { socket.disconnect(); socket.connect(); setGame(new Chess()); setRoomCode(""); setInputCode(""); setMyColor(null); setIsOwner(false); setOwnerMode(false); setGameStarted(false); setGameOver(false); setMoves([]); setCaptured({ white: [], black: [] }); setSelectedSquare(null); setPromotion(null); setClocks({ w: START_TIME, b: START_TIME, frozen: { w: false, b: false }, timeControl: DEFAULT_TIME }); setDrawOffered(false); setStatus("Create a room or join a friend's room"); }
  function formatTime(seconds) { const safe = Math.max(0, Math.ceil(Number(seconds) || 0)); return `${Math.floor(safe / 60)}:${String(safe % 60).padStart(2, "0")}`; }
  ...
  const squareStyles = useMemo(() => { if (!selectedSquare) return {}; const styles = { [selectedSquare]: { background: "rgba(255, 214, 80, 0.55)" } }; game.moves({ square: selectedSquare, verbose: true }).forEach((move) => { styles[move.to] = { background: game.get(move.to) ? "rgba(220,70,70,0.65)" : "radial-gradient(circle, rgba(60,190,100,.65) 18%, transparent 20%)" }; }); return styles; }, [game, selectedSquare]);
